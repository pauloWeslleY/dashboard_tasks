...
import { STATUS_TASK, type StatusTaskType, type TableTaskRowsProps } from './types';

export function TableTaskRows({ tasks }: TableTaskRowsProps): React.JSX.Element {
  const { columnsTableTask } = useTableTaskRows({ task: tasks });

  const StatusTask: Record<StatusTaskType, ReactElement> = {
    [STATUS_TASK.DONE]: <Tag label="Completada" iconLeft={ChecksIcon} color="success" />,
    [STATUS_TASK.NOT_DONE]: <Tag label="Pendente" iconLeft={TimerIcon} color="warning" />,
  };
          ...
          <TableCell key={task.title} sx={{ textAlign: 'center' }}>
            ...
            {typeof task.content === 'boolean' && (
              <Stack direction="row" spacing={1} sx={{ justifyContent: 'center' }}>
                {StatusTask[task.content ? STATUS_TASK.DONE : STATUS_TASK.NOT_DONE]}
              </Stack>
            )}
          </TableCell>
        );
